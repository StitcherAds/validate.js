describe("validate.async", function() {
  ...
  describe("wrapping errors", function() {
    it.promise("it allows you to wrap errors using a custom function", function() {
      ...
      const attrs = {foo: "bar", bar: "foo"}
      const originalConstraints = {foo: {numericality: true}}
      const wrapped = {attr: ["errors"]}
      const wrapper = jasmine.createSpy("wrapper").and.callFake(function(errors, options, attributes, constraints) {
      ...
      const originalOptions = {wrapErrors: wrapper};
    ...
    it.promise("calls the wrapper function with the new keyword", function() {
      // Spy on wrapper and make it behave like a constructor when called with `new`
      const wrapper = jasmine.createSpy("wrapper").and.callFake(function(errors) {
